...
- 修复积分规则保存：家长账号缺少家庭成员关系时会自动补建家庭，避免保存积分配置返回 403。
- 修复商品新增：管理端新增商品改走稳定的 `/api/shop` 接口，避免部署后旧 Server Action ID 导致提交失败。
- 修复子路径部署：客户端 API 请求会识别 `/kid` 访问路径，避免积分规则等页面保存时打到域名根路径的其它服务。
